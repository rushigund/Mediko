...
while True:
    ...
    try:
        ...
        df = pd.read_csv(r'/var/project/database/Processing/UserProcessing.txt', sep='/',float_precision='round_trip')
        print(df)
        ...
        if (df2.empty):
            ...
            df.to_csv(r'/var/project/database/Processing/UserProcessing.txt', header=True, sep='/', index=False)
            val = "Y"
            message_to_send = val.encode("UTF-8")
            clientsocket.send(len(message_to_send).to_bytes(2, byteorder='big'))
            clientsocket.send(message_to_send)
