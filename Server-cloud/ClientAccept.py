...
while True:
    ...
    print("connection form {address} has been established")
    ...
    try:
        ...
        st2 = st1.split("/")
        print(st2[0])
        df1 = pd.read_csv('/var/project/database/Processing/PData.txt', sep='/')
        Cstr = st2[1]
        Cstr = Cstr.lower()
        print(Cstr)
        ...
        if (Cstr == 'n'):
            ...
            df1.to_csv('/var/project/database/Processing/PData.txt', header=True, sep='/', index=False)
            val = "Y"
            ...
            if df2.empty:
                ...
                CN = str(CN)
                FC = lat + ',' + lan
                #with open(r'/var/project/database/Processing/UserProcessing.txt','a')as obj:
                    #obj.write(FC);
                    #obj.write('\n');

                ##dd = pd.DataFrame({'LATLAN': [FC]})
                ##dd.to_csv(r'/var/project/database/Processing/UserProcessing.txt', mode='a', header=False, index=False)
                dd = pd.DataFrame({'LAT':[lat1], 'LAN':[lan1]})
                dd.to_csv(r'/var/project/database/Processing/UserProcessing.txt',mode='a',sep='/',header=False,index=False)
