...
while True:
    ...
    try:
        ...
        st2 = st1.split("/")
        print(st2[0])
        df1 = pd.read_csv("/var/project/database/CData.txt", sep='/')
        #df2 = pd.read_csv("/var/project/database/LData.txt", sep='/')
        # print(df)
        tot = len(df1.index)
        ...
        if(dd.empty):
            ...
            df.to_csv('/var/project/database/CData.txt', mode='a', header = False, sep='/', index=False)
            dfa = pd.DataFrame({'ID': (val), 'LATITUDE': [st2[3]], 'LONGITUDE': [st2[4]]})
            dfa.to_csv("/var/project/database/LData.txt", mode='a', header = False, sep='/', index=False)
